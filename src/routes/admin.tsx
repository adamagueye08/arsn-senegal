    ...
    <div className="min-h-[70vh] bg-muted/30">
      <div className="bg-foreground text-white py-8">
        <div className="max-w-6xl mx-auto px-4 md:px-6">
          <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/60 mb-1">Accès réservé</p>
          <h1 className="text-2xl font-serif font-bold">Espace agent ARSN</h1>
        </div>
      </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="p-4 bg-white ring-1 ring-black/5 text-center">
      ...
      <div>
        <div className="flex gap-3 mb-4">
          ...
          <button
            onClick={() => load(q)}
            className="px-4 py-2 bg-foreground text-white text-sm font-semibold rounded-sm"
          >
            Rechercher
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white ring-1 ring-black/5 divide-y divide-border">
            {demandes.map((d) => (
              <div key={d.id} className="p-4 flex flex-wrap items-center gap-3 justify-between text-sm">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 bg-arsn-green text-white text-xs font-semibold rounded-sm disabled:opacity-50"
                  >
                    Valider
                  </button>
                  <button
                    ...
                    className="px-3 py-1.5 bg-arsn-yellow text-foreground text-xs font-semibold rounded-sm disabled:opacity-50"
                  >
                    Complément
                  </button>
                  <button
                    ...
                    className="px-3 py-1.5 bg-arsn-red text-white text-xs font-semibold rounded-sm disabled:opacity-50"
                  >
                    Rejeter
                  </button>
                </div>
